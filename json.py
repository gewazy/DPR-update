...
qc_s = "Select [POSTPLOT].`Station (value)`, " \
       ...
       "From [POSTPLOT] " \
       "Where [POSTPLOT].`Station (value)` > 0 And [POSTPLOT].`Track` Between 4060 And 4550 And (([POSTPLOT].`Status` IN (2,4) And [POSTPLOT].`Survey Mode (value)` Not In (3,5,6)) Or ([POSTPLOT].`Status` = 5  And  [POSTPLOT].`Survey Mode (value)` In (3) And ([POSTPLOT].`Number of Satellites` < 5 Or [POSTPLOT].`PDOP` > 6 Or [POSTPLOT].`CQ` > 0.3) )  or [POSTPLOT].`Status` = 5 or [POSTPLOT].`Status` = 6 ) " \
       "Order By [POSTPLOT].`Station (value)`"
qc_r = "Select [POSTPLOT].`Station (value)`, [POSTPLOT].`Local Easting`, [POSTPLOT].`Local Northing`, [POSTPLOT].`WGS84 Latitude`, [POSTPLOT].`WGS84 Longitude`" \
       " From [POSTPLOT] " \
...
try:
    with open('.\\output\\PL182_QC_Domiar_ZUPT.txt', 'w') as file:
        for row in qc_s:
            file.write(f"{int(row[0])} {format(round(row[3], 8), '.8f')} {format(round(row[4], 8), '.8f')} 0.00 0 7 1 0 \n")
        for row in qc_r:
            file.write(f"{int(row[0])} {format(round(row[3], 8), '.8f')} {format(round(row[4], 8), '.8f')} 0.00 0 7 1 0 \n")
except TypeError:
