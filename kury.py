...
QC_R = "Select [POSTPLOT].* " \
       "From [POSTPLOT] " \
       f"Where  [POSTPLOT].`Station (value)` > 0 And [POSTPLOT].`Track` Between {init.RECEIVERS_TRACK}  " \
...
QC_S = "Select [POSTPLOT].* " \
       "From [POSTPLOT] " \
       f"Where [POSTPLOT].`Station (value)` > 0 And [POSTPLOT].`Track` Between {init.SOURCES_TRACK}  " \
...
SKIP = "Select [POSTPLOT].* From [POSTPLOT] " \
       ...
       f"And [POSTPLOT].`Track` Between {init.SOURCES_TRACK} Order By [POSTPLOT].`Station (value)`"
